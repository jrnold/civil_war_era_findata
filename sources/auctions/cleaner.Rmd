...
bond_metadata <- get_bond_metadata("../bond_metadata.json")
...
make_auction_yields <- function(prices, date, gold_rate, metadata)
{
    ...
    if (gold_rate == 1) {
      yields1 <- sapply(prices, function(price) {
        metadata$cashflows %>%
        as.numeric(yield_to_maturity2(price, date, .))
      })
      yields6 <- yields5 <- yields4 <- yields3 <- yields2 <- yields1
    } else {
      # Yields in currency
      yields1 <- sapply(prices, function(price) {
        as.numeric(yield_to_maturity2(price * gold_rate, date, cashflows))
      })
      yields2 <- sapply(prices, function(price) {
        as.numeric(yield_to_maturity2(price, date, cashflows_currency))
      })
      yields3 <- sapply(prices, function(price) {
        as.numeric(yield_to_maturity2(price, date, cashflows_currency2))
      })
	  yields4 <- sapply(prices, function(price) {
        as.numeric(yield_to_maturity2(price, date, cashflows_currency3))
      })
    }
    data_frame(ytm1 = yields1, 
               ytm2 = yields2,
               ytm3 = yields3)
}
...
bids_1864 <- auction_1864 %>%
  ...
  bind_cols(., make_auction_yields(.$price_currency,
                                as.Date("1864-06-15"),
                                1 / GREENBACK_PRICE, 
                                bond_metadata[["us_6pct_1881_jul"]]))
...
bidders <-
  ...
  mutate(institutional = 
          grepl("(\\&|\\b(son|co(mpany)?|bro(thers?)?|insurance|bank|trustees?|savings|institution|others|cashier|president|executor)\\b)", name,
